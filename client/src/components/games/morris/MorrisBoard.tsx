    ...
    return (
        <div className="flex flex-col items-center w-full max-w-lg mx-auto px-2">
            ...
            <div className="w-full flex justify-between mb-4 sm:mb-6 md:mb-8 text-sm md:text-base font-bold text-slate-400">
                <div className={`p-2 sm:p-3 rounded-xl flex gap-2 items-center transition-all ${turn === 'player' ? 'glass-strong ring-2 ring-cyan-500' : ''}`}>
                    <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-cyan-500 shadow-[0_0_10px_#06b6d4]" />
                    <span className="text-xs sm:text-sm">YOU: {piecesHand.player > 0 ? `${piecesHand.player} to place` : piecesBoard.player}</span>
                </div>
                <div className={`p-2 sm:p-3 rounded-xl flex gap-2 items-center transition-all ${turn === 'cpu' ? 'glass-strong ring-2 ring-red-500' : ''}`}>
                    <span className="text-xs sm:text-sm">CPU: {piecesHand.cpu > 0 ? `${piecesHand.cpu} to place` : piecesBoard.cpu}</span>
                    <Cpu className="w-3 h-3 sm:w-4 sm:h-4" />
                </div>
            </div>

            <div className="relative w-[280px] h-[280px] sm:w-[340px] sm:h-[340px] md:w-[500px] md:h-[500px] bg-slate-900 rounded-xl border-4 border-slate-800 shadow-2xl p-6 sm:p-8 select-none">
                ...
                <div className="relative w-full h-full">
                    {NODES.map((node) => {
                        ...
                        return (
                            <motion.div
                                key={node.id}
                                className={`absolute w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 -ml-3 -mt-3 sm:-ml-3.5 sm:-mt-3.5 md:-ml-4 md:-mt-4 rounded-full border-2 cursor-pointer z-10 flex items-center justify-center transition-all
                ...
                <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 text-center pointer-events-none">
                    <span className="glass-strong px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-slate-200 text-xs sm:text-sm font-bold">
                        {message}
                    </span>
                </div>
            </div>
