...
// --- PLACEMENT PHASE AI ---
export const getBestPlacement = (board: (string | null)[], cpuColor: string, playerColor: string) => {
    // 1. Find all truly EMPTY spots
    const emptySpots = board.map((val, idx) => val === null ? idx : -1).filter(val => val !== -1);
...
export const getBestMove = (board: (string | null)[], cpuColor: string, playerColor: string) => {
    // 1. Find all pieces belonging to CPU
    const myPieces = board.map((val, idx) => val === cpuColor ? idx : -1).filter(val => val !== -1);
    let potentialMoves: { from: number, to: number }[] = [];
